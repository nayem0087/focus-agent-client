...
3. **Conclusion:** Scalable efficiency achieved with advanced agentic intelligence.`;
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!inputMessage.trim() || !item) return;
        ...
        try {
            const res = await fetch('/api/agent-chat', {
                ...
                body: JSON.stringify({ messages: updatedMessages, item }),
            });
            const data = await res.json();
            setMessages((prev) => [...prev, { sender: 'bot', text: data.reply }]);
        } catch (err) {
            console.error(err);
            setMessages((prev) => [...prev, { sender: 'bot', text: 'দুঃখিত, কিছু একটা সমস্যা হয়েছে।' }]);
        } finally {
            setIsTyping(false);
        }
    };
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-12 px-4 md:px-8 transition-colors duration-300">
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                <div id="ai-tools-section" className="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-6">
                    ...
                    {/* AI Chat Assistant */}
                    <div className="bg-white dark:bg-[#12141c] border border-gray-200 dark:border-gray-800 rounded-[2.5rem] p-6 md:p-8 space-y-6 shadow-sm flex flex-col justify-between h-[520px]">
                        <div className="space-y-1">
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 text-xs font-bold border border-blue-200 dark:border-blue-800">
                                <Bot size={12} /> ASK ABOUT THIS CANDIDATE
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 my-2 scrollbar-thin">
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex items-start gap-2 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`max-w-[75%] p-3 rounded-2xl text-xs leading-relaxed ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-[#1a1d29] text-gray-800 dark:text-gray-200 rounded-bl-none border border-gray-200 dark:border-gray-800'}`}>
                                        {msg.text}
                                    </div>
                                </div>
                            ))}
                        ...
                        <form onSubmit={handleSendMessage} className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                            <input
                                ...
                                className="flex-1 px-4 py-3 rounded-xl bg-gray-50 dark:bg-[#0b0c10] border border-gray-200 dark:border-gray-800 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-blue-500"
                            />
                            <button type="submit" className="px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow transition text-xs font-semibold">
                                Send
                            </button>
                        </form>
                    </div>
